...
- 2026-09-10 후속 결정: 실제 패키지 빌드/실행 검증은 나중에 일괄 진행한다. S01에서는 패키지 포함
  설정을 반영하고 요청/공식 client/응답·Feedback 계약 전환을 수행하며 실제 패키지 검증은 S04에 둔다.

## DEC-045 AI 생성 검증은 신규 세션과 여러 모델의 반복 평가로 수행

- 상태: `accepted`
- 결정일: 2026-09-10
- 결정:
  - RF-019 S04는 공식 예제 시험과 별도로 프로젝트 대화 이력이 없는 AI가 제공 자료와 사용자
    프롬프트만으로 요구 프로그램을 생성·실행할 수 있는지 검증한다.
  - 성능 중심과 비용·속도 중심의 최소 2개 모델로 기본/Teaching/GUI/변형 시나리오를 각 3회 시험한다.
  - 자료·프롬프트·도구·실행 환경을 맞추고 정확한 모델 ID와 추론 설정, 자료 버전, 코드와 시험 결과,
    추가 설명·수동 수정 여부를 기록한다. 실패 회차를 보존하고 무보조 결과와 보조 후 결과를 구분한다.
  - 제공 자료의 문제를 개선한 뒤 새 세션에서 재검증한다. 기존 대화 내용을 평가 중 보충하여 성공한
    결과를 최초 무보조 성공으로 간주하지 않는다.
  - 특정 모델의 최소 버전 이상을 보장하지 않고 실제 검증된 조합과 시나리오별 결과를 안내한다.
    모델 또는 제공 자료가 변경되면 관련 평가를 재수행한다.
- 이유: 한 모델의 추론 능력이나 우연한 생성 성공에 의존하지 않고 자료의 충분성과 재현성을 확인한다.
- 영향: RF-019 S04 완료 조건과 사용자용 검증 목록에 반영한다. 이번 결정은 평가 실행 결과가 아니다.

## DEC-046 S04 독립 실행 평가와 사전 합격 기준

- 상태: `accepted`
- 결정일: 2026-09-10
- 대체 관계: DEC-045를 보완하여 평가 환경·판정·보고 기준을 구체화한다.
- 결정:
  - 모델과 Agent/도구/환경 조합을 평가하고 매회 세션·작업 폴더·Mock 상태를 초기화한다.
  - 생성 전에 요구사항별 시험·합격 기준·자원 예산을 고정한다. 공개 예제와 별도로 문서 개선에
    쓰지 않은 변형을 포함하고 평가 도구·서버는 생성 AI가 수정할 수 없도록 한다.
  - Agent 자체 시험·수정은 예산 내 허용하되 최종 판정은 독립 실행 시험으로 수행한다.
    최초/최종 무보조/사람 보조 결과와 환경 오류를 분리하고 실패 기록을 보존한다.
  - 모든 적용 필수 항목이 통과해야 회차 합격이다. 코드 미관·생성 비용으로 동작 실패를 상쇄하지 않는다.
  - 3회 반복은 초기 관측이며 일반 신뢰성 보장이 아니다. 3/3 통과 범위를 명시하고 부분 성공을 구분한다.
    전체 모델이 실패한 필수 시나리오는 해결 전 S04를 완료하지 않는다.
- 이유: 제공 자료의 충분성과 생성 결과의 실제 동작을 평가하고 테스트 오염·자기 판정·작은 표본의
  과도한 해석을 피한다.
- 영향: 상세 시험표, 수행 절차, 참고 자료와 보고 기준은 RF-019 S04를 따른다.

## DEC-047 사용자 commissioning과 시퀀스 생성 Agent의 책임 분리

- 상태: `accepted`
- 결정일: 2026-09-10
- 결정: 사용자가 하드웨어 설치·배선·commissioning, config.txt/.env 설정과 Motion Server 정상 구동을
  담당한다. 시퀀스 생성 Agent는 정상 구동된 서버의 기존 API로 요구 프로그램을 생성·시험한다.
  설정은 읽기 전용 구성 자료로 사용하고 정상 구동 문제는 사용자에게 보고한다.
- 이유: 장치/서버 준비와 애플리케이션 생성의 책임을 명확히 한다.
- 영향: RF-019 프롬프트와 S04 환경은 정상 구동 서버가 준비된 전제다. 개발 Agent의 S01 서버 코드
  구현과 사용자용 시퀀스 생성 Agent의 역할을 혼동하지 않는다.
