...
- 사용자에게 하드웨어 설치·commissioning·config.txt/.env 설정·서버 정상 구동 책임을 두었다.
- Agent는 설정을 읽어 정상 서버의 API를 사용하는 프로그램을 생성하며 S04도 준비된 서버에서 시작한다.
  DEC-047과 RF-019에 반영했다.

### RF-019 S04 시험·평가 기준 보완

- 외부 평가 가이드/연구 교차 확인을 바탕으로 독립 실행 시험, 평가 전 기준·예산 고정,
  환경 초기화와 미사용 변형 시나리오를 S04에 추가했다(DEC-046).
- 요구 순서·API·입력 대기·Stop/Fail·Teaching·GUI·책임 경계별 필수 합격 기준을 작성했다.
- 최초 산출물/자체 수정 후 무보조/사람 보조 결과와 환경 오류를 분리하며, 3/3 관측을 일반 신뢰성
  보장으로 사용하지 않도록 했다. 참고 자료 링크를 남겼다. 시험 실행 없이 문서만 갱신했다.

### RF-019 S04 신규 AI 세션 및 다중 모델 검증 추가

- 공식 예제 실행과 별도로 제공 자료만 사용하는 새 AI 세션의 생성·Mock 실행 평가를 추가했다.
- 최소 2개 모델, 기본/Teaching/GUI/변형 4개 시나리오, 각 3회 반복과 동일 도구·환경 조건을 기록했다.
- 모델·추론 설정·자료 버전·프롬프트·코드·시험 로그·수동 개입 및 실패 회차를 보존하고, 자료 개선 후
  새 세션에서 재검증하도록 했다. 최소 버전 제한 대신 검증된 모델 목록을 제공하기로 했다(DEC-045).
- 문서만 갱신했으며 모델 평가를 실행하거나 통과를 선언하지 않았다.
